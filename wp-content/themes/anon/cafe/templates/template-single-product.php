...
if ($the_query->have_posts()):
    while ($the_query->have_posts()): $the_query->the_post();
        if (!post_password_required()) {
            ...
            remove_action('woocommerce_after_single_product', 'zoo_sticky_add_to_cart', 5);
            remove_action( 'woocommerce_single_product_summary', 'zoo_single_product_extend_notice', 35 );
            remove_action( 'woocommerce_single_product_summary', 'zoo_single_product_meta', 5 );
            remove_action('woocommerce_after_add_to_cart_button', 'zoo_button_wishlist', 10, 0);
            remove_action('woocommerce_after_add_to_cart_button', 'zoo_button_products_compare', 10, 0);
            remove_action( 'woocommerce_single_product_summary', 'zoo_wc_get_stock_html', 0 );
            remove_action( 'woocommerce_after_add_to_cart_button', 'zoo_buy_now_button', 15 );
            zoo_before_single_product();
            ?>
